...
int conectar_con_swap(){
	...
	if (enviar_mensaje(sock, msg)>0){
		log_info(log_general_p, "SWAP - Mensaje de confirmacion enviado correctamente");
	}
...
int swap_nuevo_proceso(int pid, int nro_paginas){
	...
	log_info(log_general_p, "SWAP - Pedido reserva: Proceso %d - Paginas %d",pid,nro_paginas);

	msg = recibir_mensaje(socket_swap);
	if(msg!=NULL){
		if(msg->header.id == SWAP_OK){
			log_info(log_general_p, "SWAP - Reserva correcta");
			destroy_message(msg);
			return 0;
		}
		log_info(log_general_p, "SWAP - No se pudo reservar %d paginas para el proceso %d",nro_paginas,pid);
		log_error(log_errores, "SWAP - No se pudo reservar %d paginas para el proceso %d",nro_paginas,pid);
		destroy_message(msg);

	}else{
		log_info(log_general_p, "SWAP - No se pudo reservar por problemas de comunicacion");
...
char* swap_leer_pagina(int pid, int pagina){
	...
	enviar_y_destroy_mensaje(socket_swap, msg);
	log_info(log_general_p, "SWAP - Pedido lectura: Proceso %d - Pagina %d",pid,pagina);
	msg = recibir_mensaje(socket_swap);
	...
	if(msg!=NULL){
		if(msg->header.id == SWAP_OK){
			...
			log_info(log_general_p, "SWAP - Lectura correcta: %s",contenido);
			destroy_message(msg);
			return contenido;
		}else{
			log_info(log_general_p, "SWAP - Lectura incorrecta");
			log_error(log_errores, "SWAP - Lectura incorrecta");
		}
		destroy_message(msg);
	}else{
		log_info(log_general_p, "SWAP - Lectura incorrecta por problemas de comunicacion");
...
int swap_escribir_pagina(int pid, int pagina, char* contenido){
	...
	enviar_y_destroy_mensaje(socket_swap, msg);
	log_info(log_general_p, "SWAP - Pedido escritura: Proceso %d - Pagina %d - Contenido %s",pid,pagina,contenido);

	msg = recibir_mensaje(socket_swap);

	if(msg!=NULL){
		if(msg->header.id == SWAP_OK){
			log_info(log_general_p, "SWAP - Escritura correcta");
			destroy_message(msg);
			return 0;
		}else{
			log_info(log_general_p, "SWAP - Escritura incorrecta");
			log_error(log_errores, "SWAP - Escritura incorrecta");
		}
		destroy_message(msg);
	}else{
		log_info(log_general_p, "SWAP - Escritura incorrecta por problemas de comunicacion");
...
int swap_finalizar(int pid){
	...
	enviar_y_destroy_mensaje(socket_swap, msg);
	log_info(log_general_p, "SWAP - Pedido finalizacion: Proceso %d",pid);

	msg = recibir_mensaje(socket_swap);

	if(msg!=NULL){
		if(msg->header.id == SWAP_OK){
			log_info(log_general_p, "SWAP - Finalizacion correcta");
			return 0;
		}else{
			log_info(log_general_p, "SWAP - Finalizacion incorrecta");
			log_error(log_errores, "SWAP - Finalizacion incorrecta");
		}
		destroy_message(msg);
	}else{
		log_info(log_general_p, "SWAP - Finalizacion incorrecta por problemas de comunicacion");
