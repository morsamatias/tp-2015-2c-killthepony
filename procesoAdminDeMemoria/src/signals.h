...
//int gl_pagina;
	...
	/*	for(i=0;i<CANTIDAD_MARCOS();i++)
				memoria[i] = NULL;
					...
					//PAGINAS ES LA LISTA CON LOS PROCESOS
					list_iterate(paginas,(void*)es_el_proceso_segun_PID);
                 //AHORA TENGO QUE BUSCAR POR LA LISTA DE PAGINAS ADENTRO DEL PROCESO QUE ES PAGINAS
...
void limpiar_tabla_de_paginas_por_proceso(t_pagina* pagina){


	if(pagina->modificado){


		swap_escribir_pagina(pagina->PID,pagina->pagina,memoria[pagina->marco]->contenido );
...
void limpiar_tabla_de_paginas(t_proceso* proceso){

	list_iterate(proceso->paginas,(void*)limpiar_tabla_de_paginas_por_proceso);
